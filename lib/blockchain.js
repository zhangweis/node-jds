...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  this.init = function init() {
    Step(
      ...
      function importFromStep(err) {
        if (err) throw err;

        if (self.cfg.storage.importFrom) {
          // TODO: Implement static import
          this(null);
        } else {
          this(null);
        }
  ...
  storage.getOutputsByInputs.bind(storage);

  var getQueueCount = this.getQueueCount = function getQueueCount() {
    return incomingBlockQueue.length;
  };

  var add = this.add = function add(block, txs, raw, callback) {
    var self = this;
    ...
    var bw = new BlockWrapper(block, txs, raw);
    if (callback) {
  ...
  var processBlock = this.processBlock = function processBlock(bw) {
    ...
    Step(
      ...
      function prepareTxsStep(err) {
        ...
        if (!self.isPastCheckpoints()) {
          // Until the last checkpoint we only verify tx hashes which
          // happens entirely in C++.
          this(null);
          return;
        } else if (!bw.txs) {
          // In all other cases we parse the transactions now. This
          // data will be freed once the block is done processing
          // meaning we should only ever have one block deserialized
          // in memory.
          var parser = new Parser(bw.raw);

          // Skip block header
          parser.seek(80);
          var txCount = parser.parseVarInt();
          bw.txs = [];
          for (i = 0; i < txCount; i++) {
            bw.txs.push(parser.parseTx());
          }
        }
  ...
  var saveBlock = this.saveBlock = function saveBlock(bw, callback)
  {
    ...
    storage.appendBlockQuick(bw.block, bw.raw, cb);
  };
...
var BlockWrapper = exports.BlockWrapper = function BlockWrapper(block, txs, raw) {
  this.block = block;
  this.txs = txs;
  this.raw = raw;
  this.callbacks = [];
