...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  var processBlock = this.processBlock = function processBlock(bw) {
    ...
    Step(
      ...
      function verifyBlockStep(err) {
        ...
        if ((!self.cfg.verify) || Buffer.isBuffer(bw.txs)) {
          this();
          return;
        }
      ...
      function endTransactionStep(err) {
        ...
        // In quick add mode the db transaction is finalized automatically
        if (Buffer.isBuffer(bw.txs)) {
          this(null);
          return;
        }
  ...
  var saveBlock = this.saveBlock = function saveBlock(bw, callback)
  {
    ...
    if (Buffer.isBuffer(bw.txs)) {
      storage.appendBlockQuick(bw.block, bw.txs, cb);
    } else {
      storage.saveBlock(bw.block, cb);
    }
  };
