...
Connection.prototype.processData = function () {
  ...
  var message;
  try {
    message = Connection.parseMessage(command, payload, this.node.isQuickDownload());
  } catch (e) {
...
/**
 * Turn a binary Bitcoin message into a JS object.
 *
 * @param {string} command The command string associated with the payload.
 * @param {Buffer} payload The actual data to be parsed
 * @param {bool} rawTxs If true, returns the raw payload in the 'txs' field for
 *  the 'block' message rather than parsing the transactions. This option is
 *  used for performance reasons during the initial blockchain download.
 */
Connection.parseMessage = function (command, payload, rawTxs) {
  ...
  case 'block':
    data = Connection.parseBlockHeader(parser);
...
Connection.parseBlockHeader = function (parser)
{
