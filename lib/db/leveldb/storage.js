...
var BitcoinLevelDB = require('../../binding').LevelDB;
...
function LeveldbStorage(uri) {
  ...
  function (inputs, callback) {
    ...
    Step(function () {
      var txList = [];
      var txList64 = [];
      inputs.forEach(function (txinBuf) {
        var hash = txinBuf.slice(0, 32);
      ...
      getOutputsByHashes(txList, this);
    }, function (err, txs) {
      if (err) throw err;

      // Create a temp index by hash
      var txIndex = [];
      txs.forEach(function (tx) {
      ...
      // Find any requested outputs that are fulfilled by this set of txs
      inputs.forEach(function (txinBuf, n) {
        var hash = txinBuf.slice(0, 32);
        var hash64 = hash.toString('base64');
        if (txIndex[hash64]) {
          var txin = new TransactionIn({o: txinBuf});
          var outIndex = txin.getOutpointIndex();
          outputs[n] = txIndex[hash64].outs[outIndex];
        }
      });
      this(null, outputs);
    }, callback);
