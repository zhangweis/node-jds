...
function LeveldbStorage(uri) {
  ...
  var connect = this.connect = function connect(callback) {
    ...
    Step(
      ...
      function createBlockPrevIndexDb(err, db) {
        if (err) throw err;

        self.hMain = hMain = db;

        leveldb.open(prefix+'blockprev.db', defaultCreateOpts, this);
      },
        function createBlockHeightIndexDb(err, db) {
        if (err) throw err;

        self.hBlockPrevIndex = hBlockPrevIndex = db;
        
        leveldb.open(prefix+'blockheight.db', defaultCreateOpts, this);
      },
      function createBlockTxsIndexDb(err, db) {
        if (err) throw err;
  ...
  function emptyDatabase(callback) {
    Step(
      ...
      function (err) {
        if (err) throw err;
        leveldb.destroy(prefix+'main.db', {});
        leveldb.destroy(prefix+'blockprev.db', {});
        leveldb.destroy(prefix+'blockheight.db', {});
        leveldb.destroy(prefix+'blocktx.db', {});
        leveldb.destroy(prefix+'affects.db', {});
        this(null);
      },
      function (err) {
        if (err) throw err;
  ...
  function getTopBlock(callback) {
    ...
    Step(
      ...
      function (err) {
        ...
        var hash = iterator.value(defaultGetOpts);
    ...
    addrHashes.forEach(function (addrHash) {
      ...
      steps.push(function iterateStep(err) {
        ...
        while (Buffer.isBuffer(key) &&
          ...
          iterator.next();
          key = iterator.key(defaultGetOpts);
        }
        this(null, hashes);
      });
    });
