...
var BitcoinLevelDB = require('../../binding').Database;

var Block = require('../../schema/block').Block;

var txSchema = require('../../schema/transaction');
var Transaction = txSchema.Transaction;
var TransactionIn = txSchema.TransactionIn;
var TransactionOut = txSchema.TransactionOut;
...
function deserializeBlock(data) {
  return new Block(Connection.parseBlockHeader(data));
};
...
function applyBlockMetadata(block, data) {
  ...
  block.chainWork = new Buffer(data.chainWork, 'binary');
  block.txs = data.txs.map(function (hash) {
...
function LeveldbStorage(uri) {
  ...
  var connect = this.connect = function connect(callback) {
    ...
    Step(
      ...
      function initMetadata(err, db) {
        ...
        self.hMain = hMain = db;
        self.bldb = bldb = new BitcoinLevelDB(db.self);
  ...
  var setMeta = this.setMeta = function (key, value, callback) {
    ...
    if ("string" === typeof key) {
      metadata[key] = value;
      steps.push(function () {
        wb.put(key, JSON.stringify(value));
        this(null);
      });
  ...
  this.saveBlock = function (block, callback) {
    var wb;
    try {
      wb = currentBatch ? currentBatch : hMain.batch();
      var hash = block.getHash();
      var data = serializeBlock(block);
      var meta = getBlockMetadata(block);

      // Save block header
      wb.put(hash, data);

      // Save block metadata
      var metaKey = new Buffer(36);
      hash.copy(metaKey);
      metaKey.write("meta", 32);
      wb.put(metaKey, meta);

      // Save block height index
  ...
  this.appendBlockQuick = function (block, raw, callback) {
    var hash = block.getHash();
    ...
    Step(function () {
      ...
      // This will complete the WriteBatch
      bldb.addBlock(raw, wb.self, this);
      currentBatch = null;
    }, callback);
  };
  ...
  function connectTransactions(txs, callback) {
    Step(
      function saveSpent() {
        ...
        // Save affected addresses index
        var keyPrefix = new Buffer("affects", "ascii");
        txs.forEach(function (tx) {
          if (tx.affects) {
            tx.affects.forEach(function (affect) {
              var key = keyPrefix.concat(affect, tx.getHash());
              wb.put(key, '');
            });
          }
        });
  ...
  var getOutputsByInputs = this.getOutputsByInputs =
  function (inputs, callback) {
    var outputs = bldb.getOutputsByInputs(inputs);
    outputs = outputs.map(function (txoutBuf) {
      if (txoutBuf === null) return null;

      var parser = new Parser(txoutBuf);
      return new TransactionOut(parser.parseTxOut());
    });
  ...
  function getBlockByHash(hash, callback) {
    ...
    Step(
      function getBlockStep() {
        ...
        hMain.get(hash, defaultGetOpts, this);
      }, function getBlockMetadataStep(err, data) {
        if (err) throw err;
        ...
        if (data) {
          ...
          hash.copy(metaKey);
          metaKey.write("meta", 32);
          hMain.get(metaKey, defaultGetOpts, this);
        } else {
          this(null, null);
        }
      }, function postStep(err, data) {
        if (err) throw err;

        if (data) {
          applyBlockMetadata(block, data);
        }
  ...
  function getBlocksByHashes(hashes, callback) {
    async.map(hashes, getBlockByHash, callback);
  };
    ...
    Step(
      function () {
        ...
        for (var i = 0, l = heights.length; i < l; i++) {
          hMain.get(formatHeightKey(heights[i]), defaultGetOpts, group());
        }
      },
      function (err, hashes) {
        if (err) throw err;
  ...
  function getContainingBlock(txHash, callback)
  {
    Step(
      function queryBlockTxsIndexStep() {
        ...
        key.write('txblock');
        txHash.copy(key, 7);
        hMain.get(key, defaultGetOpts, this);
      },
      callback
    );
  };
  ...
  function getAffectedTransactions(addrHashes, callback)
  {
    ...
    addrHashes.forEach(function (addrHash) {
      var key = keyPrefix.concat(addrHash);
      var hashes = [];
      ...
      steps.push(function iterateStep(err) {
        ...
        function getNextKey(err) {
          ...
          iterator.key(defaultGetOpts, function (err, key) {
            if (Buffer.isBuffer(key) && key.slice(7, 27).equals(addrHash)) {
              hashes.push(key.slice(27));
