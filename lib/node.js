...
Node.prototype.handleBlock = function (e) {
  ...
  this.blockChain.add(block, txs, raw, callback);
};
...
Node.prototype.handleBlockAddCallback = function (err) {
  ...
  if (err) {
    if (err instanceof VerificationError) {
      var message = "Block verification result negative\n";
      message += "-> In block " + Util.formatHashFull(block.hash) + "\n";
      if (err.txHash) {
        message += "-> In tx " + Util.formatHashFull(err.txHash) + "\n";
      }
      message += "-> " + (err.stack ? err.stack : err.toString());
      logger.error(message);
    } else {
      // Generic error
      logger.error("Block verification error "+
                   Util.formatHashFull(block.hash)+"\n"+
                   "-> " + (err.stack ? err.stack : err.toString()));
    }
  }
};
...
Node.prototype.isQuickDownload = function () {
  return !this.blockChain.isPastCheckpoints();
};
