...
Node.prototype.handleGetblocks = function (e) {
  ...
  self.blockChain.getBlockByLocator(e.message.starts, function (err, latestCommonBlock) {
    ...
    var nextHeight = latestCommonBlock.height + 1;
    ...
    (function nextBlock() {
      self.blockChain.getBlockByHeight(nextHeight++, function(err, block) {
        ...
        // should anything be returned in this case?
        if (err) {
        ...
        // if this block is not active, can we expect others to be active?
        if (!block || !block.active) {
          return nextBlock();
        }

        // done gathering blocks
        // either hit the limit of 500 or the stop hash
        if (blocks.length === 500 || block.getHash().equals(e.message.stop)) {
