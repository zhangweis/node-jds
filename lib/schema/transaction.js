...
Transaction.prototype.verify = function verify(txCache, blockChain, callback) {
  ...
  function getTxOut(txin, n) {
    var txout = txCache.get(n);

    if (!txout) {
      var outIndex = txin.getOutpointIndex();
      throw new MissingSourceError("Source output index "+outIndex+
                                   " for input "+n+" not found",
                                   txin.getOutpointHash());
    }
  ...
  Step(
    function verifyInputsStep() {
      var group = this.group();

      if (self.isCoinBase()) {
        throw new VerificationError("Coinbase tx are invalid unless part of a block");
      }
    ...
    function verifyInputsResultsStep(err, results) {
      if (err) throw err;

      for (var i = 0, l = results.length; i < l; i++) {
        if (!results[i]) {
          var txout = getTxOut(self.ins[i], i);
    ...
    function checkConflicts(err, count) {
      ...
      if (valueIn.cmp(valueOut) < 0) {
        ...
        throw new VerificationError("Tx output value (BTC "+outValue+") "+
  ...
  if (!(this.affects && this.affects.length)) {
    ...
    for (var i = 0, l = this.ins.length; i < l; i++) {
      try {
        ...
        // instead.
        var txout = txCache.get(i);
        if (!txout) {
          throw new Error("Corresponding output not found!");
        }
        var script = txout.getScript();
...
TransactionInputsCache.prototype.buffer = function buffer(blockChain, txStore, wait, callback)
{
  ...
  Step(
    ...
    function findMemTx() {
      if (txStore) {
        var txList64 = [];
        self.txins.forEach(function (txin) {
        ...
        txStore.find(txList64, this);
      } else {
        this(null, []);
      }
    },
    // Populate the index using a set of tx objects
    function indexTxStep(err, txs) {
      if (err) throw err;
      ...
      self.txins.forEach(function (txin, n) {
        var hash = txin.o.slice(0, 32);
        var hash64 = hash.toString('base64');
        if (txIndex[hash64]) {
          var outIndex = txin.getOutpointIndex();
          self.txouts[n] = txIndex[hash64].outs[outIndex];
        }
      });
    ...
    function findBlockChainTx(err) {
      ...
      blockChain.getOutputsByInputs(self.txins.map(function (txin) {
        return txin.serialize();
      }), this);
    },
    function finalizeStep(err, result) {
      if (err) throw err;

      result.forEach(function (txout, n) {
        if (txout) {
          self.txouts[n] = txout;
        }
      });
...
TransactionInputsCache.prototype.get = function get(index)
{
  return this.txouts[index];
};
